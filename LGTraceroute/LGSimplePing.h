...
@protocol LGSimplePingDelegate;

/*! Controls the IP address version used by LGSimplePing instances.
 */

typedef NS_ENUM(NSInteger, LGSimplePingAddressStyle) {
    LGSimplePingAddressStyleAny,          ///< Use the first IPv4 or IPv6 address found; the default.
    LGSimplePingAddressStyleICMPv4,       ///< Use the first IPv4 address found.
    LGSimplePingAddressStyleICMPv6        ///< Use the first IPv6 address found.
};
...
@interface LGSimplePing : NSObject
...
@property (nonatomic, weak, readwrite, nullable) id<LGSimplePingDelegate> delegate;

/*! Controls the IP address version used by the object.
 *  \details You should set this value before starting the object.
 */

@property (nonatomic, assign, readwrite) LGSimplePingAddressStyle addressStyle;
...
/*! A delegate protocol for the LGSimplePing class.
 */

@protocol LGSimplePingDelegate <NSObject>

@optional

/*! A LGSimplePing delegate callback, called once the object has started up.
...
/*! A LGSimplePing delegate callback, called if the object fails to start up.
 *  \details This is called shortly after you start the object to tell you that the 
...
/*! A LGSimplePing delegate callback, called when the object has successfully sent a ping packet. 
...
- (void)simplePing:(LGSimplePing *)pinger didSendPacket:(NSData *)packet sequenceNumber:(uint16_t)sequenceNumber;

/*! A LGSimplePing delegate callback, called when the object fails to send a ping packet. 
 *  \details Each call to `-sendPingWithData:` will result in either a 
 *      `-simplePing:didSendPacket:sequenceNumber:` delegate callback or a 
 *      `-simplePing:didFailToSendPacket:sequenceNumber:error:` delegate callback (unless you 
 *      stop the object before you get the callback).  These callbacks are currently delivered 
...
- (void)simplePing:(LGSimplePing *)pinger didFailToSendPacket:(NSData *)packet sequenceNumber:(uint16_t)sequenceNumber error:(NSError *)error;

/*! A LGSimplePing delegate callback, called when the object receives a ping response.
...
/*! A LGSimplePing delegate callback, called when the object receives an unmatched ICMP message.
 ...
 *  
 *      This is declared in the header because clients of LGSimplePing might want to use 
