...
#pragma mark * LGSimplePing

@interface LGSimplePing ()
...
- (void)didFailWithError:(NSError *)error {
    id<LGSimplePingDelegate>  strongDelegate;
 ...
 *  \returns The offset of the ICMP header, or NQDotFound.
 */
...
- (void)readData {
    ...
    if (bytesRead > 0) {
        ...
        NSString *srcAddr = [self srcAddrInIPv4Packet:packet];
        
        strongDelegate = self.delegate;
        if ( [self validatePingResponsePacket:packet sequenceNumber:&sequenceNumber] ) {
            if ( (strongDelegate != nil) && [strongDelegate respondsToSelector:@selector(simplePing:didReceivePingResponsePacket:sequenceNumber:srcAddr:)]) {
                [strongDelegate simplePing:self didReceivePingResponsePacket:packet sequenceNumber:sequenceNumber srcAddr:srcAddr];
            }
        } else {
...
static void SocketReadCallback(CFSocketRef s, CFSocketCallBackType type, CFDataRef address, const void *data, void *info) {
    ...
    obj = (__bridge LGSimplePing *) info;
    assert([obj isKindOfClass:[LGSimplePing class]]);
        ...
        for (NSData * address in addresses) {
            ...
            if ( address.length >= sizeof(struct sockaddr) ) {
                switch (addrPtr->sa_family) {
                    case AF_INET: {
                        if (self.addressStyle != LGSimplePingAddressStyleICMPv6) {
                            self.hostAddress = address;
                            resolved = true;
                        }
                    } break;
                    case AF_INET6: {
                        if (self.addressStyle != LGSimplePingAddressStyleICMPv4) {
                            self.hostAddress = address;
                            resolved = true;
                        }
                    } break;
